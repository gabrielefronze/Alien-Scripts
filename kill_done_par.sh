...
# extract failed not-master process ids
cat $FILEPATH | grep "D" | grep -o '\-[0-9]\{9\}' | cut -c 2- > completedslavejobs.txt

FAILED="$(grep -c ^ completedslavejobs.txt)"
...
# kills done slave jobs using alien_kill
while read line; do 
